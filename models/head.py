...
class AdaFace(Module):
    def __init__(self,
        ...
        # self.batch_mean = self.batch_mean.('cuda')
        # self.register_buffer = self.register_buffer.('cuda')
    ...
    def forward(self, embbedings, norms, label):
        ...
        # print(kernel_norm.size())
        cosine = torch.mm(embbedings,kernel_norm.to('cuda'))
        ...
        with torch.no_grad():
            mean = safe_norms.mean().detach().to('cuda')
            std = safe_norms.std().detach().to('cuda')
            # print(type(self.batch_mean))
            # print(type(self.batch_std))
            self.batch_mean = self.batch_mean.to(device="cuda")
            self.batch_std = self.batch_std.to(device="cuda")
